...
// 
// #define OBSERVABLE_TESTS
// #define DISPATCH_TESTS
...
namespace Tests
{
    ...
    {
        public static void RunTests(string etlFileName, TextWriter log)
        {
            log.WriteLine("Start ObservableTests");
            using (var source = new ETWTraceEventSource(etlFileName))
            {
                var startCallbackCount = source.CallbackCount();
                log.WriteLine("StartCallbackCount = " + startCallbackCount);
                ...
                var cnt = 0;
                using (var gcSub = Subscribe(gcTicks, gcTickData => log.WriteLine("Got Tick {0}", gcTickData.AllocationAmount), () => log.WriteLine("Ticks Completed.")))
                using (var manifestSub = Subscribe(perfViewTicks, manifestData => log.WriteLine("Got PerfView Tick {0:f4}", manifestData.TimeStampRelativeMSec), () => log.WriteLine("Manifests Completed")))
                using (var allTasksSub = Subscribe(allTasks, delegate(TraceEvent allTasksData) { 
                    if (allTasksData.EventName != "ManifestData") 
                        log.WriteLine("Got AllTasks: Data = {0}", allTasksData);
                }, () => log.WriteLine("allTasks Completed")))
                using (var logSub = Subscribe(logMessages, logMessageData => log.WriteLine("Got PerfView Log Message {0}", logMessageData.PayloadByName("message")), () => log.WriteLine("Log Messages Completed")))
                {
                    ...
                    allPerfSub = Subscribe(allPerfView,
                         delegate(TraceEvent allPerfViewData)
                         {
                             ...
                             if (cnt >= 5)
                             {
                                 log.WriteLine("Canceling allPerfiew");
                                 allPerfSub.Dispose();
                                 allPerfSub = null;
                             }
                             log.WriteLine("allPerfView {0}", allPerfViewData);

                         },
                         () => log.WriteLine("allPerfView Completed"));
                    source.Process();
                ...
                log.WriteLine("endCallbackCount = " + endCallbackCount);
            }
...
#region events
    ...
    public class DispatcherTester
    {
        ...
        public void ObservableTests(TextWriter log)
        {
            ...
            using (Subscribe(events1, d => log.WriteLine("1 Got event 1"), () => log.WriteLine("1 Completed")))
            using (Subscribe(events1, d => log.WriteLine("2 Got event 1"), () => log.WriteLine("2 Completed")))
            using (Subscribe(all, d => log.WriteLine("ALL Got event NUM {0}", d.ID), () => log.WriteLine("ALL Completed")))
            using (Subscribe(unhandled, d => log.WriteLine("UNHANDLED Got event NUM {0}", d.ID), () => log.WriteLine("UNHANDLED Completed")))
            {
            ...
            Debug.Assert(endCallbackCount == originalCallBackCount);
            Console.WriteLine("Done");
        }

        public void DispatcherTests(TextWriter log)
        {
            var asParserServices = (ITraceParserServices)m_dispatcher;
            var curCallbackCount = m_dispatcher.DistinctCallbackCount();
            log.WriteLine("Callback Count {0}", curCallbackCount);
            var r = new Random(10);
            ...
            for (int i = 1; i < 50000; i++)
            {
                ...
                Debug.Assert(m_dispatcher.TemplateLength() <= 1024);
                ...
                if (m_activeTemplates.Count < 470 && (m_activeTemplates.Count == 0 || r.Next(100) < 51))
                {
                    if (r.Next(5) != 0 || m_activeTemplates.Count == 0)
                    {
                        ...
                        Debug.Assert(newCallbackCount == curCallbackCount + 1);
                    ...
                    else
                    {
                        ...
                        asParserServices.RegisterEventTemplate(newTemplate);
                        var newCallbackCount = m_dispatcher.DistinctCallbackCount();
                        Debug.Assert(newCallbackCount == curCallbackCount);
                        curCallbackCount = newCallbackCount;
                    }
                }
                else
            ...
            log.WriteLine("Callback Count {0}", m_dispatcher.DistinctCallbackCount());
        }
        ...
        unsafe void RunTest()
        {
            ...
            foreach (var template in m_inactiveTemplates)
                Debug.Assert(!m_visited.ContainsKey(template));
        ...
        {
            Debug.Assert(!m_visited.ContainsKey(data));
